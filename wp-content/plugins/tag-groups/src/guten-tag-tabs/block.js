...
class TagGroupsHelp extends Component {
  constructor() {
    ...
    if (hasPremium) {
      ...
      this.helpComponent = 'tabbed-tag-cloud/tabbed-tag-cloud-gutenberg-block/';
    } else {
...
class tagGroupsTabbedCloudParameters extends Component {
  ...
  toggleOptionAdjustSeparatorSize() {
    let adjust_separator_size = this.props.attributes.adjust_separator_size
      ? 0
      : 1;
  ...
  toggleOptionAddPremiumFilter(key) {
    let add_premium_filter = this.props.attributes.add_premium_filter ? 0 : 1;
    this.props.setAttributes({ add_premium_filter });
  }

  toggleOptionHideEmptyTabs() {
    let hide_empty_tabs = this.props.attributes.hide_empty_tabs ? 0 : 1;
    this.props.setAttributes({ hide_empty_tabs });
  }

  toggleOptionShowTabs() {
    let show_tabs = this.props.attributes.show_tabs ? 0 : 1;

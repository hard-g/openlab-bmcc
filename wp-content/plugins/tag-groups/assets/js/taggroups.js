/**
 * Last modified: 2020/12/21 18:31:58
  ...
  jQuery.ajax({
    ...
    success: function(data) {
      try {
        ...
        message_output =
          '<div class="notice notice-error"><p>Error loading groups. Please check <b>Tag Groups Settings -> Troubleshooting -> System Information -> Ajax Test</b></p></div><br clear="all" />';
      ...
      if (status === 'success') {
        ...
        if (max_number > 0) {
          for (var key in groups) {
            ...
            if (data_set.id != null) {
              output +=
                '<tr class="tg_sort_tr" data-position="' + position + '">\n';
              output += '<td>' + data_set.id + '</td>\n';
              output +=
                '<td class="tg_label_column"><span class="tg_edit_label tg_text" data-position="' +
                position +
                '" data-label="' +

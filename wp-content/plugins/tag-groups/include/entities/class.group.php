...
if ( !class_exists( 'TagGroups_Group' ) ) {
    class TagGroups_Group
    {
         ...
         * @return object
         */
        public function create( $label, $position = null )
        {
        ...
        )
        {
             ...
             */
            $cache_key = md5( $this->group_id . '-' . serialize( $taxonomy ) . '-' . serialize( $hide_empty ) . '-' . strtolower( $fields ) . '-' . $post_id . '-' . strtolower( $orderby ) . '-' . strtolower( $order ) . '-' . $pll_post_language . '-' . $wpml_language );
            $transient_value = TagGroups_Transients::get_transient( TagGroups_WPML::get_tag_groups_group_terms_transient_name() );
            $natural = false;
            
            if ( $transient_value === false || !isset( $transient_value[$cache_key] ) ) {
                ...
                if ( !in_array( $orderby, $default_orderby ) ) {
                ...
                if ( !empty($terms) ) {
                    ...
                    $transient_value[$cache_key] = $terms;
                    TagGroups_Transients::set_transient( TagGroups_WPML::get_tag_groups_group_terms_transient_name(), $transient_value, 7 * DAY_IN_SECONDS );
                }
                
                return $terms;
            } else {
                return $transient_value[$cache_key];
            }

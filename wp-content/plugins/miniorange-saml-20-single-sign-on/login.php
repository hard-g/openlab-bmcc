...
class saml_mo_login {
		...
		if ( current_user_can( 'manage_options' ) ) {
			...
			if ( isset( $_POST['mo_feedback'] ) and $_POST['mo_feedback'] == 'mo_feedback' ) {
				...
				if ( $deactivate_reason ) {
					...
					update_option( 'mo_saml_message', 'Please Select one of the reasons ,if your reason is not mentioned please select Other Reasons' );
					$this->mo_saml_show_error_message();
				}
	...
	function miniorange_sso_menu() {
		...
		$page = add_menu_page( 'MO SAML Settings ' . __( 'Configure SAML Identity Provider for SSO', 'mo_saml_settings' ), 'miniOrange SAML 2.0 SSO', 'administrator', 'mo_saml_settings', array(
			$this,

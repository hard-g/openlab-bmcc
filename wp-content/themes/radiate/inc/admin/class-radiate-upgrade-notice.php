...
class Radiate_Upgrade_Notice extends Radiate_Notice {
	...
	public function __construct() {
		...
		$dismiss_url = wp_nonce_url(
			add_query_arg( 'radiate_notice_dismiss', 'upgrade', admin_url() ),
			'radiate_upgrade_notice_dismiss_nonce',
			'_radiate_upgrade_notice_dismiss_nonce'
		);

		$temporary_dismiss_url = wp_nonce_url(
			add_query_arg( 'radiate_notice_dismiss_temporary', 'upgrade', admin_url() ),
			'radiate_upgrade_notice_dismiss_temporary_nonce',
			'_radiate_upgrade_notice_dismiss_temporary_nonce'
		);
	...
	private function set_notice_time() {
		if ( ! get_option( 'radiate_upgrade_notice_start_time' ) ) {
			update_option( 'radiate_upgrade_notice_start_time', time() );
		}
	}

	private function set_temporary_dismiss_notice_time() {
		if ( isset( $_GET['radiate_notice_dismiss_temporary'] ) && 'upgrade' === $_GET['radiate_notice_dismiss_temporary'] ) {
			update_user_meta( $this->current_user_id, 'radiate_upgrade_notice_dismiss_temporary_start_time', time() );
		}
	}
	...
	public function set_dismiss_notice() {
		...
		if ( get_option( 'radiate_upgrade_notice_start_time' ) > strtotime( '-5 day' )
			...
			add_filter( 'radiate_upgrade_notice_dismiss', '__return_true' );
		} else {
			add_filter( 'radiate_upgrade_notice_dismiss', '__return_false' );
		}
	}

	public function notice_markup() {
		?>
		<div class="notice notice-success radiate-notice">
			<a class="radiate-notice-dismiss notice-dismiss" href="<?php echo esc_url( $this->dismiss_url ); ?>"></a>
			...
			<div class="links">
				<a href="<?php echo esc_url( $this->pricing_url ); ?>" class="button button-primary" target="_blank">
					...
					<span><?php esc_html_e( 'Upgrade to pro', 'radiate' ); ?></span>
				</a>
				...
				<a href="https://themegrill.com/contact/?utm_source=radiate-dashboard-message&utm_medium=button-link&utm_campaign=pre-sales" class="button button-secondary" target="_blank">
					<span class="dashicons dashicons-info"></span>
					<span><?php esc_html_e( 'Got pre sales queries?', 'radiate' ); ?></span>
				</a>
			</div>
		</div> <!-- /radiate-notice -->
		<?php
